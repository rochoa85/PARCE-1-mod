...
class complex:
    ########################################################################################
    def __init__(self,binder,pdbID,iteration,score_list,consensus_threshold,t_effective,num_mutations,scoring_s,target_type,score_mode="all",sim_time="10",mode="start",try_mutations=10):
        """
    ...
    def get_molecules_after_md(self,md_name="md-npt"):
        ...
        if self.target_type=="protein":
            # Center the xtc file using the target and the protein as reference
            rc,sout,serr=gromacs.trjconv(f=self.path+"/{}.xtc".format(md_name),s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",pbc="nojump", o=self.path+"/NOJUMP.xtc",stdout=False,input=("0"))
            rc,sout,serr=gromacs.trjconv(f=self.path+"/NOJUMP.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",o=self.path+"/CENTER.xtc",center=True,stdout=False,input=("Protein \n 0"))
            rc,sout,serr=gromacs.trjconv(f=self.path+"/CENTER.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",pbc="mol",ur="compact",o=self.path+"/npt-pbc.xtc",stdout=False,input=("0"))
            rc,sout,serr=gromacs.trjconv(f=self.path+"/CENTER.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",pbc="mol",b="{}".format(str(self.time_ref)),ur="compact",o=self.path+"/npt-pbc.gro",stdout=False,input=("0"))
        
        if self.target_type=="drug":
            # Center the xtc file using the target and the protein as reference
            rc,sout,serr=gromacs.trjconv(f=self.path+"/{}.xtc".format(md_name),s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",center=True,pbc="mol",ur="compact", o=self.path+"/npt-pbc.xtc",stdout=False,input=("target \n 0"))
            rc,sout,serr=gromacs.trjconv(f=self.path+"/{}.xtc".format(md_name),s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",center=True,pbc="mol",ur="compact",b="{}".format(str(self.time_ref)),o=self.path+"/npt-pbc.gro",stdout=False,input=("target \n 0"))
        ...
        # Get the complex
        rc,sout,serr=gromacs.trjconv(f=self.path+"/npt-pbc.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",b="{}".format(str(self.time_ref)),
                                     o=self.path+"/complexP/complexP_"+str(self.iteration)+".pdb",stdout=False,input=("complex"))
    ...
    def read_score_trajectory(self,pdb_file,score_list,half_flag,only_complex=False):
        ...
        score_dictionary={}
        for sc in self.score_list: score_dictionary[sc]=0.0
        #score_dictionary={'pisa':0.0,'bach':0.0,'firedock':0.0,'irad':0.0,'totalbmfbluues':0.0,'bmf':0.0,'bluues':0.0,'zrank':0.0,'vina':0.0}
        total_score={}
        for sc in self.score_list: total_score[sc]=[]
        #total_score={'pisa':[],'bach':[],'firedock':[],'irad':[],'totalbmfbluues':[],'bmf':[],'bluues':[],'zrank':[],'vina':[]}
                    ...
                    for s in score_list:
                        ...
                        if s=="dligand":
                            if only_complex:
                                ...
                                sc.computeDligand()    
                                total_score[s].append(float(sc.dligand_score))
                        if s=="dsxscore":
                            if only_complex:
                                total_score[s].append(0.0)
                            else:
    ...
    def score_complex(self,score_list,half_flag,md_name="md-npt"):
        ...
        if self.score_mode=="all":
            ...
            rc,sout,serr=gromacs.trjconv(f=self.path+"/npt-pbc.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",
                                         o=self.path+"/all_npt_traj.pdb",stdout=False,input=("complex"))
            # Target trajectory
            rc,sout,serr=gromacs.trjconv(f=self.path+"/npt-pbc.xtc",s=self.path+"/{}.tpr".format(md_name),n=self.path+"/molecules.ndx",
            ...
            self.sc_dict_complete={}
            for sc in self.score_list: self.sc_dict_complete[sc]=0.0    
    ...
    ########################################################################################
    def mutation_random(self,residues_mod,mutation_document,score_dictionary,half_flag,mutation_method,scwrl_path,target):
        """
        ...
        for i in range(0,self.num_mutations):
            ...
            if self.scoring_s=="single":
                accept=mutated_system.metropolis_criteria(score_dictionary,last_good_iteration,mutated_system.score_list)
