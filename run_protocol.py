...
if __name__ == '__main__':
    ...
    if args.mode in ("start","restart","nothing"):
        ...
        exit()
    if args.target in ("protein","drug"):
        target=args.target
    else:
        print("The parameter 'target' is required for the analysis, or an option should be selected from: protein or drug. Exiting ...")
        exit()
    ...
    if args.score_list:
        ...
        if scoring_s=="consensus":
            ...
            if target=="protein":
                for s in score_list:
                    if s not in ("bach","pisa","zrank","irad","bmf-bluues","firedock"):
                        print("The scoring function {} is not suitable for a protein target. Exiting ...".format(s))
                        exit()
            if target=="drug":
                for s in score_list:
                    if s not in ("vina","smina","cyscore","nnscore","dligand","dsxscore"):
                        print("The scoring function {} is not suitable for a drug target. Exiting ...".format(s))
                        exit()
    else:
    ...
    protein_complex.mutation_random(residues_mod,mutation_document,score_dictionary,half_flag,mutation_method,scwrl_path,target)
